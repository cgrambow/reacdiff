function S = mydecorrstretch(A, relativeStretch, useCorr)
%use relative stretch (a three-valued array) to specify the magnitude of stretching in each principle components
if nargin < 2
  relativeStretch = ones(3,1);
end
if nargin < 3
  useCorr = true;
end
...
[T, offset]  = fitdecorrtrans(meanB, cov, useCorr, targetMean, [], relativeStretch);
...
function [T, offset] = fitdecorrtrans(means, Cov, useCorr, targetMean, targetSigma, relativeStretch)
...
if useCorr
    ...
    [V D] = eig(Corr);
    T = pinv(S) * V * decorrWeight(D, relativeStretch) * V' * targetSigma;
else
    [V D] = eig(Cov);
    T = V * decorrWeight(D, relativeStretch) * V' * targetSigma;
end
...
%--------------------------------------------------------------------------
function W = decorrWeight(D, relativeStretch)
...
D(D < 0) = 0;
W = sqrt(pinv(D)) .* diag(relativeStretch);
end
