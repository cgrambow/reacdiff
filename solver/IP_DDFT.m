...
forwardHandle = @(tdata,y0,FSA,meta,params) forwardSolver(tdata,y0,FSA,meta,params,tspan,ybound);
assignHandle = @(name,xparam,params) assign(name,xparam,params,Cspace,kernelSize);
switch mode
case 'eval'
...
case 'IP'
  ...
  forwardHandle, @adjointSolver,loss,lossHess, assignHandle,'discrete',discrete), ...
  x_guess, options);
case 'sens'
  ...
  forwardHandle, [],loss,lossHess, ...
  @(name,xparam,params) assign(name,xparam,params,Cspace,kernelSize,suppress),'discrete',discrete);
case 'pp'
  x_opt = [];
  fval = [];
  exitflag = [];
  params = paramsTransfer(x_guess,meta,params,assignHandle,ydata,false);
end
